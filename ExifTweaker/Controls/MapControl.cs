...
public sealed partial class MapControl : UserControl
{
    ...
    public async Task SetThemeAsync(bool darkTheme)
    {
        browser.DefaultBackgroundColor = darkTheme ? Color.FromArgb(30, 30, 30) : Color.White;
        if (browser.CoreWebView2 is not null)
            await browser.ExecuteScriptAsync($"setTheme({JsonSerializer.Serialize(darkTheme ? "dark" : "light")});");
    }
